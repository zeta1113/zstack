...
VERSION 슬롯이 현재 열린 PR에 의해 주장된다, 이는 지휘자 작업 공간이 WIP가 곧 발송 가능성이, 어떤 슬롯 /ship 다음을 선택 할 것. 아니 mutations - 그냥 스냅 샷. " 착륙 보고서", "그것에서 큐", "내가 PR을 열, 또는 "이 버전이 다음 주장 할 것"을 물었다.

# /landing-report - 버전 큐 대시보드
...
5-10 평행한 지휘자 작업대를 달리면, 그것은 — 한 눈에 볼 것을 돕습니다 — 어느 버전 번호가 주장하고, 다음 `/ship`가 땅에 뒀습니다. 이 기술은 동일한 `bin/gstack-next-version` 실용 `/ship` 용도에 읽기 전용 호출, 그러나 어떤 돌연변이도 없이. `gh pr list`를 위해 VERSION 수를 위해 `gh pr list`로 생각하십시오.

---

## 단계 1: 플랫폼과 기본 branch을 검출

다른 gstack 기술과 동일한 탐지.
...
3 번의 util을 호출 - 각 범프 레벨에 대해 한 번 - 그래서 사용자는 그들이 주장하는 것을 볼 수 micro/patch/minor/major. 저렴한 (same gh는 bun에 의해 캐시).
...
단일 테이블 출력을 빌드하십시오. 큐 + siblings (충전 레벨의 동일)에 대한 canonical로 `patch`-level JSON를 사용하십시오. `.version`는 다릅니다).

`jq`를 사용하여 추출:
- `.host` - github | gitlab | 알 수 없음
- `.offline` - 쿼리가 실패 했습니까?
- `.claimed` — {pr, branch, 버전, URL의 배열}
- `.siblings` — 모두 sibling worktrees 발견
- `.active_siblings` - 배에 대해 가능성이 있는 하위 설정
...
오프라인 / 알 수없는 호스트 출력의 경우, 더 짧은 블록을 인쇄하십시오.
...
테이블을 렌더링 한 후, ONE을 제안합니다.

1. **queue에 충돌이 있다면** (두 개의 열린 PR은 동일한 버전을 주장):
   "⚠ 두 개의 열린 PR은 v<X>에 콜라겐을 엽니다. 두 번째 합병은 첫 번째 CHANGELOG 항목 또는 토지 복제를 덮어 두 번 겹쳐 쌓입니다. 다음 무료 슬롯을 선택하려면 /ship를 다시 실행하도록 한 저자를 묻습니다.

2. **Active sibling이 user의 분기 버전을 실행하면:**
   "작업 트리 <path>은 v<X>를 <N>h 전적으로 유지하고 아직 PR'd를 사용하지 않습니다. 그 작업이 처음 출하되면, 지점은 착륙 시간에 다시 부풀릴 필요가 있습니다."

3. **모든 것이 깨끗하게 보이는 경우:**
   "Queue는 깨끗합니다. 다음 /ship는 충돌없이 슬롯을 주장합니다."

---

## 계획 형태

PLAN MODE EXCEPTION — ALWAYS RUN. 이 기술은 완전히 읽기 전용입니다: 파일 쓰기 없음, git mutations 없음, 네트워크 상태 변화 없음. 계획 형태에서 달리는 안전.
