...
Diataxis 프레임 워크 (tutorial / how-to / reference / description)를 사용하여 완전한 구조 문서 생성. 그것은 적용 간격을 찾을 때 /document-release에 의해 호출 된 독립 또는 호출 할 수 있습니다. "docs"를 작성할 때 "generate documentation", "document this feature", "create tutorial", 또는 "explain this module"을 "createplain.
...
# 문서 생성자: Diataxis 문서 작성자

`/document-generate` 워크플로우를 실행하고 있습니다. 작업: **높은 품질, 구조화 된 문서**를 생성, 모듈, 또는 전체 프로젝트. 단일 문서의 단일 줄을 작성하기 전에 코드를 철저히 연구합니다.

이 기술은 두 가지 방법을 호출 할 수 있습니다:
1. **의 특징** — 사용자는 기능, 단위, 또는 프로젝트에서 당신을 점하고 “이 문서”를 말합니다
2. **/document-release에서** - 커버리지 맵은 갭을 식별; 당신은 그들을 채울

**Diataxis 프레임 워크**를 따르십시오 - 문서의 4개의 사분자, 각 서빙 다른 독자 필요:
- **의논하기** - 학습 중심, 작업 예제 단계별 예제를 통해 새로운 커서를 걸어
- **으로** - 작업 중심, 특정 목표를 달성하는 방법을 보여줍니다 (기본 친숙성)
- **Reference** - 정보 중심, 완전하고 정확한 기술 설명
- **의약** - 이해 지향, 그 일을 하는 방법을 설명

**철학: 연구 전체, 그 후에 부속을 씁니다.** 단일 방을 그리기 전에 전체 사이트를 조사 한 건축가처럼 문서 작성하기 전에 전체 코베이스 표면을 읽습니다. 이것은 "기능 절반을 설명하는 문서" 실패 모드를 방지합니다.
...
1. 문서에 대한 정의:
   - **특정 대상에 부합하는 경우** (feature, module, file, Skill): 범위는 대상이다.
   - **전체 프로젝트에 대 한 호출 하는 경우**: 범위는 가득 차있는 프로젝트입니다
   - **/document-release에서 갭으로 호출하면**: 범위는 적용 지도에서 특정한 entities입니다

2. AskUserQuestion를 사용하여 범위를 확인하고 문서 대상에 대해 묻습니다.

   - A) 기존 파일에 있는 문서 인라인 작성 (README, ARCHITECTURE, 등)
   - B) 독립 문서 파일 생성 (예: `docs/` 디렉토리)
   - C) 두 — 기존 파일의 개요 summaries + 독립 파일에 깊은 docs

   RECOMMENDATION: C를 선택하여 검색성과 깊이를 극대화합니다.

3. 출력 형식을 결정하십시오:
   - 프로젝트가 이미 `docs/` 디렉토리를 가지고 있다면, 그 규칙을 따르십시오.
   - 프로젝트가 doc 프레임워크 (Nextra, Docusaurus, MkDocs, VitePress)를 사용한다면 형식을 따르십시오.
   - Otherwise, use plain Markdown files in `docs/`

---

## 단계 1: Codebase 고고학 (연구 단계)

**이것은 가장 중요한 단계입니다.** 건너뛰지 않거나 멈춰도. 문서의 품질은 코드를 잘 이해하는 방법에 대한 직접 비례입니다.
...
2. **입력을 읽으십시오.** 식별 및 읽기:
   ...
   - package.json / Cargo.toml / pyproject.toml / go.mod (프로젝트 유형에 따라)
   - 주요입력파일(index.ts, main.rs, app.py, cmd/main.go)
   - 구성 파일 및 예

3. **각 대상인에 대한 소스 코드를 읽으십시오.** 각 기능의 경우/module 당신은 문서화:
   - 구현 파일 종료 (만 서명하지 않음)
   - 테스트 읽기 - 그들은 행동, 가장자리 케이스 및 사용 패턴을 밝혀
   - 대상이에 따라 달라지는 관련 모듈을 읽어 보거나 이에 따라 달라집니다.
   - 기존 인라인 주석을 읽으십시오, 특히 `// NOTE:`, `// DESIGN:`, `// WHY:`

4. **컨셉 맵을 구축합니다.** 쓰기 전에, 내부 개요를 생성합니다:
...
5. 출력 : "연구 N 파일, K 공개 표면 항목, M 개념 및 J 디자인 결정."

---

## 단계 2: Diataxis 분할

각 대상인 경우, Diataxis 생성에 대한 보조금을 결정합니다. 모든 법인이 4개가 필요하지 않습니다.

**결정 모체:**

| Entity 유형 | 자습서? | 어떻게 되나요? | 참고 사항 | 설명 |
|---|---|---|---|---|
| 새로운 기능으로 사용자의 상호 작용 | ✅ | ✅ | ✅ | - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
| CLI 명령 또는 플래그 | - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - | ✅ | ✅ | No |
| 내부 모듈/architecture | No | No | ✅ | ✅ |
| Config 옵션 | No | ✅ | ✅ | No |
| 디자인 패턴 / 철학 | No | No | No | ✅ |
| API 엔드포인트 | - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - | ✅ | ✅ | No |
| 워크플로우(다단계 프로세스) | ✅ | ✅ | No | - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - |
...
계획이 생성하는 5개 이상의 서류가 있는 경우, 진행하기 전에 AskUserQuestion를 사용하세요. 작은 범위의 경우, 직접 진행하십시오.

---

## Step 3: 제보 문서 작성

참고 문서는 기초입니다. 그들은 사실적이고, 완료하고 코드에서 직접 파생됩니다. 튜토리얼이나 어휘를 설정하기 때문에이 글을 작성하십시오.
...
**참고 문서의 규칙:**
- 우아함에 대한 정확도. 모든 클레임은 코드에 추적되어야한다.
- 유형, 기본 및 제약이 포함됩니다. "문자 문자열을 받아들이십시오"는 충분합니다. "Accepts a
  문자열 (최대 256 숯, `^[a-z-]+$`)"은 참고 등급입니다.
- 실제로 작업 할 수있는 실제 예제를 보여줍니다. copy-pasted.
- *왜?*를 설명하지 마십시오.
...
**설명 docs에 대한 규칙:**
- 문제로 리드, 솔루션이 아닙니다.
- ASCII 을 구성하는 다이어그램을 사용합니다. 그들은 grep-able, diff-friendly, 어디에나 렌더링합니다.
- 이름 거래 오프 명시적으로. "우리는 Z가 "가 금 표준이기 때문에 Y 이상 X를 선택했습니다.
- 참조 자료가 반복되지 않습니다. - 링크가 있습니다.

---

## Step 5: 가이드를 읽는 방법

어떻게 작업 지향. 그들은 독자가 기본을 알고 뭔가 특정을 수행하고 싶어.
...
**docs에 대한 규칙 :**
- 제목은 "How to"로 시작 - 예외가 없습니다. 이것은 독자의 항목 포인트입니다.
- 모든 단계는 동작해야합니다. "consider if..."- 대신 "Run X"또는 "Z로 Y 추가"
- 검증을 포함. 독자는 "그것을 작동하지 않는?"
- 문제 해결 섹션은 작업이 실패 할 수 있는지 필수입니다.

---

## Step 6: 튜토리얼을 작성

튜토리얼은 학습 중심입니다. 그들은 0에서 작업 예로의 새로운 기능을 가지고 있습니다. 이것은 잘 쓰고 가장 귀중한 가장 어려운 것입니다.
...
**튜토리얼 규칙 :**
- **처음 < 3 단계.** 리더가 3단계로 일하는 것을 본 적이 없는 경우
  튜토리얼은 너무 느립니다.
- 각 단계는 눈에 보이는 변화 또는 산출을 일으키야 합니다. 보여주는 없이 "지금은 X를" 구성하지 않습니다
  어떤 변화.
- 정확한 명령을 사용하여 독자가 입력합니다. " 적절한 명령"초록을 실행하지 마십시오.
- 오류 경로: 단계가 일반적으로 실패하면 오류와 수정 인라인을 보여줍니다.
- "What you built"와 함께 종료 - 실제 사용 사례로 다시 튜토리얼을 연결.

---

## Step 7: 교차 문 연결 및 발견성

모든 문서를 작성한 후:

1. **사만간의 교차링크 추가** 모든 참고 문서는 그 방법에 연결해야 합니다.
   모든 방법 - 참조에 링크를해야합니다. 자습서는 모두에 링크를해야합니다.

2. **입력-포인트 파일을 업데이트합니다.** 새 문서에 대한 참조를 추가합니다.
   - README.md - 문서 섹션 또는 내용의 테이블에 추가
   - CLAUDE.md / AGENTS.md - 관련 프로젝트 구조에 추가
   - 기존의 docs 인덱스 또는 sidebar config

3. **발견 가능성을 검증합니다.** 모든 새 문서는 2개의 클릭 안에 도달할 수 있어야 합니다
   README.md. docs 프레임워크가 사용중인 경우, sidebar/nav config에 추가합니다.

4. **끊긴 링크 확인.** 어떤 `](`를 위한 Grep는, 존재하지 않는 파일에 점.

---

## 단계 8: 질 각자 Review

이 기준에 대해 각 문서를 커밋하기 전에:

**정확도 문:**
- [ ] 모든 코드 예제 컴파일 / 실행 / 복사-pasted 경우 패스
- [ ] 모든 API 설명은 실제 코드 서명에 일치
- [ ] 표시된 모든 명령은 출력을 출력한다.
- [ ] 이름이 지정된 /removed entities에 stale 참고 없음

**완료 문:**
- [ ] 참고 docs 덮개 100%년의 공중 표면
- [ ] 사용자의 상위 3개의 작업을 어떻게 덮는 것은 시도할 것입니다
- [ ] 튜토리얼은 ≤3 단계의 작업 결과에 도착
- [ ] Explanation docs 이름 거래 떨어져, 뿐만 아니라 선택

**음성 문:**
- [ ] 코드를 본 적이 없는 스마트한 사람에 대한 글
- [ ] 처음 사용에 간단한 인라인 광택없이 항아리
- [ ] Active voice, 콘크리트 명사, 짧은 문장
- [ ] "당신은 지금 할 수 있습니다 ..."하지 "시스템은 제공 ..."

진행하기 전에 모든 실패를 수정합니다.

---

## 단계 9: Commit & 산출

1. name (never `git add -A` 또는 `git add .`)에 의해 새로운 문서 파일을 단계.

**작업하기 전에 Redaction 검사.** 일반 문서는 예의 자격 증명을 포함; 단계별 문서 내용을 스캔하고 HIGH 자격 증명에 블록 (작용된 문서의 라이브 형식 비밀은 누출입니다). 예의 구성은 ` ```example ` fences won't excuse a live-format secret, but the per-span placeholder filter passes obvious docs examples (e.g. `AKIAIOSFODNN7EXAMPLE`)에 속합니다.
...
4. **PR가 존재하면**, PR체를 `## Documentation Generated`섹션 목록으로 만들기로 업데이트
   Diataxis 사각형과 한 줄 설명과 함께 새로운 파일:
...
- **쓰기 전에 연구.** 단계 1는 선택적이지 않습니다. 코드를 읽고, 시험을 읽으십시오, 읽으십시오
  기존의 docs. 충분한 연구는 표면 수준 문서를 생성합니다.
- **정확도는 비 양도할 수 있습니다.** 모든 코드 예제가 작동해야합니다. 모든 API 설명은 일치해야합니다
  실제 코드. 세부 사항에 대해 불확실한 경우, 다시 소스를 읽으십시오 — 추측하지 마십시오.
- **Diataxis 사만은 다른 독자를 봉사합니다.** 참고 docs로 튜토리얼 콘텐츠를 섞지 마십시오.
  또는 참고 내용에 대한 방법. 각 사분에는 특정 모드에서 특정 리더가 있습니다.
- **튜토리얼에서 첫 번째 결과에 시간.** 리더가 3단계로 일하는 것을 볼 수 없는 경우
  튜토리얼을 재창조합니다.
- **크로스 링크 모든 것.** 고립된 문서는 undiscoverable docs입니다.
- **음성: 친절하고, 콘크리트, 사용자를 위해.** 스마트한 사람에게 설명하는 것 같은 글
  누가 코드를 본 적이 없습니다. 결코 기업, 결코 학문적.
- **최소화에 대한 완전성.** AI는 포괄적인 문서를 싼 만듭니다. 씁니다
  "분수 바이러스성 문서"- 작성 완전한 문서. 바다를 끓입니다.
