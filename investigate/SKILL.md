...
1. **증상을 수집:** 오류 메시지, 스택 추적 및 재생 단계 읽기. 사용자가 충분한 컨텍스트를 제공하지 않은 경우, AskUserQuestion을 통해 시간에 ONE 질문을하십시오.

2. **코드를 읽으십시오:** symptom에서 잠재적인 원인으로 코드 경로 추적. 모든 참조를 찾기 위해 Grep를 사용, 논리를 이해 읽으십시오.
...
5. **조사 역사 검사:** 같은 파일에 조사를 위한 사전 학습을 검색합니다. 동일한 지역에 있는 버그를 재발하는 것은 건축 냄새입니다. 사전 조사가 존재하는 경우, 참고 본 및 루트 원인이 구조화 된 경우 확인하십시오.
...
출력 : **"루트 원인 hypothesis : ..."** - 무엇이 잘못되고 왜 특정, 테스트 가능한 주장.

###는 저명한 저명한 학습을

top-of-skill 학습 pull 위는 "debug investigation"로 크게 키워집니다. 이제 특정 hypothesis를 가지고 있으며, 같은 문제 모양 표면의 이전 수정을 저하시키는 데 키워진 재-pull 학습이 있습니다.

ONE 키워드를 hypothesis에서 선택합니다. 키워드는 명료되어야 합니다: 실패 성분 이름, 당신이 의심한 파일의 기본 이름 ( 연장 없이), 또는 버그 noun. 키워드 MUST 알파핵 또는 하이픈 만 - no 인용, 슬픔, 도트, 식민지, 또는 whitespace. 당신의 후보자가 그들 중 하나가 있는 경우에, 단지 알파벳 줄기로 간단하게 합니다.

작업 예 (인베스티게이트 별) : 좋은 키워드는 `auth-cookie`, `session-expiry`, `redirect-loop`입니다. 나쁜 : `auth.ts:47`, `fix the auth bug`, `<hypothesis-keyword>`.

```bash
~/.claude/skills/gstack/bin/gstack-learnings-search --query "<your-keyword>" --limit 5 2>/dev/null || true
```

어떤 학습이 돌아 오는 경우, 하나는 한 문장에서 조사에 적용 된 이름. none가 다시되면, 참고없이 계속 - 일치하는 사전 학습의 부재는 자체 유용한 정보입니다.

---

## 범위 자물쇠

루트 원인 hypothesis를 형성 한 후, 영향을받는 모듈에 잠금을 편집하여 범위를 크립을 방지합니다.
...
**FREEZE_AVAILABLE:** 영향을 받은 파일이 포함된 좁은 디렉토리를 식별합니다. 동결 상태 파일에 쓰기:
...
`<detected-directory>`를 실제 디렉토리 경로(예: `src/auth/`)로 구성합니다. 이 디버그 세션을 위해 `<dir>/`에 제한된 "Edits를 말합니다. 이 변경은 관련 코드에 대한 변경을 방지합니다. `/unfreeze`를 실행하여 제한을 제거하십시오."

버그가 전체 repo 또는 범위가 진짜로 아저씨가 될 경우, 잠금을 건너 왜 참고하십시오.

**FREEZE_UNAVAILABLE:** 범위를 차단합니다. 편집은 제한되지 않습니다.
...
| 제품 정보 | 의 특징 | 어디를 볼 |
|---------|-----------|---------------|
| 레이스 상태 | Intermittent, 타이밍 의존 | 공유 상태에 대한 Concurrent 접근 |
| Nil/null 전파 | NoMethodError, 유형Error | 선택 값에 대한 미스링 가드 |
| 국가 corruption | Inconsistent 자료, 부분 업데이트 | 거래, 콜백, 후크 |
| 통합 실패 | Timeout, 예상치 못한 응답 | 외부 API 통화, 서비스 경계 |
| 구성 drift | 로컬로 작동, staging/prod에서 실패 | Env vars, 기능 플래그, DB 상태 |
| Stale 캐시 | 오래된 데이터를 표시, 캐시에 수정 | Redis, CDN, 브라우저 캐시, 터보 |

또한 체크:
- `TODOS.md` 관련 알려진 문제
- `git log` 같은 지역에 대한 사전 수정 - **같은 파일에 벌레를 재귀는 건축 냄새입니다**, coincidence

**외부 본 수색:** 버그가 위의 알려진 패턴과 일치하지 않는 경우, WebSearch for:
- "{framework} {generic error type}" - **첫 번째를 질화:** strip hostnames, IPs, 파일 경로, SQL, 고객 데이터. 오류 범주를 검색하고, 원시 메시지가 아닙니다.
- "{library} {component} 알려진 문제"

WebSearch가 사용할 수 없는 경우, 이 검색을 건너서 hypothesis 테스트를 진행합니다. 문서화된 솔루션 또는 알려진 의존성 버그 표면이 있다면, 3단계의 후보가 저하로 존재합니다.
...
2. **hypothesis가 잘못되면 :** 다음 저하를 형성하기 전에 오류를 검색하는 것을 고려하십시오. **첫 번째** - 스트립 호스트 이름, IPs, 파일 경로, SQL 파편, 고객 식별자 및 오류 메시지에서 내부/proprietary 데이터. 일반적인 오류 유형 및 프레임 워크 컨텍스트만 검색하십시오. "{component} {sanitized error type} {framework version}". 오류 메시지가 안전하게 손상되기 때문에 검색을 건너뛰십시오. 웹 사이트가 취소되지 않으면 서서 반환되지 않습니다. 더 많은 증거를 가리키십시오. 추측하지 마십시오.

3. **3 strike 규칙:** 3개의 가설이 실패한 경우에, **STOP**. AskUserQuestion를 사용하십시오:
   ```
...
**레드 플래그** — 이 중 하나를 볼 경우, 느려지게:
- "지금 빠른 수정"- 거기에 no "지금에 대한. 그것은 올바른 또는 에스컬레이터를 수정.
...
4. **전체 테스트 스위트를 실행합니다.** 출력을 붙여 넣는다. No 회귀 허용.

5. **수정이 >5 파일이 있는 경우:**는 폭발 반경을 기리기 위하여 AskUserQuestion를 이용합니다:
   ```
...
미래의 세션을 위한 학습으로 조사를 이동합니다. `type: "investigation"`를 사용 하 여 영향을 받는 파일 그래서 미래의 조사 같은 지역에 찾을 수 있습니다:
...
- **수정 터치 >5 파일 → AskUserQuestion** 진행하기 전에 폭발 반경에 관하여.
- **완료 상태:**
  - DONE — root cause found, fix apply, regression test 작성, 모든 테스트 패스
  - DONE_WITH_CONCERNS - 고정하지만 완전히 검증할 수 없습니다 (예 : 간헐적 버그, 노화가 필요)
  - BLOCKED — 조사 후에 뿌리 원인 불순, 에스컬레이션
