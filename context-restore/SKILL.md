...
최근 저장된 상태로로드 (현재 branch을 우선, 지점을 가로 질러 다시) 그래서 당신은 당신이 왼쪽을 픽업 할 수 있습니다 - 도체 워크스페이스 핸드 오프. "resume", "restore context", "내가 내가"또는 "내가 왼쪽으로"라고 물어. /context-save와 쌍. 전적으로 /checkpoint 이력서 - 현재 환경에서 기본 재바람 별의로 /checkpoint를 치료하기 때문에 이름을 변경.
...
# /context-restore - 저장 작업 컨텍스트를 복원

**직원 엔지니어는 동료의 기적 세션 노트를 읽습니다.**는 그들이 떨어져 떠난 곳에 정확하게 픽업하기 위하여 입니다. 당신의 일은 가장 최근 저장된 상황에 적재하고 그(것)들을 명확하게 이렇게 사용자는 잃지 않고 일할 수 있습니다.

**HARD GATE:** Do NOT 코드 변경을 구현합니다. 이 기술은 저장 된 컨텍스트 파일을 읽고 요약을 제시합니다.

**기본: CURRENT 분기에 저장된 가장 최근의 체크포인트를 선호합니다. 이 지점이 없다면 ALL 분기에 가장 최근의 체크포인트를 떨어뜨립니다.** 방사능은 지휘자 workspace handoff를 위해 입니다 — 1개의 branch에서 저장된 컨텍스트는 다른에서 재개될 수 있습니다. 현재 벤치 선호는 1개의 체크포인트 디렉토리 (same Origin-derived slug)를 공유하는 각 worktree가 그것 `/context-restore` 없이, 침묵적으로 worktree의 더 새로운 검문소를 적재할 수 있었습니다.

**NOT hard-filter 현재 branch에 있는 후보자 세트를 하십시오** - 다른 브레이크 체크 포인트는 가을으로 세트에 체재합니다. 그들은 다만 주문됩니다 *후후후* 현재 branch의 소유, 그래서 현재 브레이크 득점은 더 새로운 sibling-worktree 득점방해에 의해 결코 shadowed. (`/context-save list`는 1 branch에 단단한스코프입니다.)
...
- `/context-restore` → 가장 최근 저장된 컨텍스트를로드하십시오 (모든 branch)
- `/context-restore <title-fragment-or-number>` → 특정 저장된 상황에 적재
- `/context-restore list` → 사용자 "사용 `/context-save list` — 목록
  저장 측에 생활" 및 출구. 여기에서 형태 탐지 없음.

---

## 복원 흐름

### Step 1: 저장된 컨텍스트를 찾아보세요
...
**후보자는 디렉토리에 `.md` 파일이 포함됩니다.**, 그러나 그들은 **현재-branch-first** (분은 각 파일 `branch:` frontmatter에서 읽습니다) 명령됩니다. 다른 벤치 파일은 현재 branch가 그것의 자신의 검문소가 없는 경우에, 도체 작업 공간 handoff를 보존하는 가을 뒤로 놓아 놓아 놓아 놓습니다.

### 단계 2: 오른쪽 파일을로드

- 사용자 지정 제목 파편 또는 번호: 중 일치하는 파일을 찾습니다
  의논문
- 그렇지 않으면: **첫 번째 파일이 단계 1 위에 반환**를 로드한다.
  최신 `YYYYMMDD-HHMMSS` 현재 branch의 체크포인트, 또는, 현재 branch이 none이 있다면, 모든 지점의 최신.

선택한 파일을 읽고 요약을 제시하십시오.
...
현재 branch이 저장된 컨텍스트의 지점과 다를 경우, "이 컨텍스트는 지점 `{branch}`에 저장되었습니다. 현재 `{current branch}`에 있습니다. 계속하기 전에 지점을 전환 할 수 있습니다."

### Step 3: 다음 단계 제공

현재 진행중인 후 AskUserQuestion을 통해 요청하십시오.

- A) 나머지 품목에 계속 일
- B) 전체 저장 파일보기
- C) 그냥 맥락, 감사를 필요로

A가 첫 번째 나머지 작업 항목을 요약하고 거기 시작을 제안한다.

---

## 저장되지 않은 상황에 처하지 않는 경우

1 단계가 `NO_CHECKPOINTS`를 인쇄하면, 사용자를 말합니다.

"저장된 컨텍스트가 없습니다. `/context-save`를 실행하면 현재 작업 상태를 저장하고 `/context-restore`가 찾을 수 있습니다."

---

## 중요 규칙

- **비밀번호** 이 기술만 저장된 파일을 읽고 그(것)들을 선물합니다.
- **현재 branch의 자체 체크포인트를 예매하지만 모든 지점을 유지
  fallback set.** Cross-branch 이력서 (Conductor Handoff)는 현재 branch이 체크포인트가 없는 경우도 여전히 작동하며, 더 이상 worktree의 newer 득점을 하게 되어 브랜치가 자신의 지점을 저장합니다.
- **"최근"는 파일 이름을 의미합니다 `YYYYMMDD-HHMMSS` 접두사**, 아니
  `ls -1t` (filesystem mtime). 파일 이름은 파일 시스템 운영에서 안정적입니다. mtime은 아닙니다.
- **gstack 기술이 아닌 Claude Code 내장.** 사용자 유형이면
  `/context-restore`, 기술 도구를 통해이 기술을 호출합니다.
