...
첫 번째 호출 새로운 의도 프로토 타입은 $B primitives를 통해 흐름을 반환 JSON. 일치 의도한 경로에 대한 호출은 통합 된 브라우저 - 스킬에 반환 ~200ms. 읽기 전용 - mutating 흐름 (형식 채우기, 클릭, 제출), 사용 /automate. "scrape", "get data from", "pull", "extract from", 또는 "what"페이지에서 ".
...
웹에서 데이터를 얻기위한 한 항목 포인트. 두 번의 두 가지 경로:

1. **경기 경로** (~200ms) - 사용자가 기존에 일치하면
   브라우저 스킬의 트리거는 `$B skill run <name>`를 통해 실행하고 JSON를 방출합니다.
2. **Prototype 경로** (~30s) - 일치하는 기술이 아직 없으므로 페이지를 구동하십시오.
   `$B` primitives로 JSON를 돌려주고, `/skillify`를 제안해, 다음의 호출은 일치 경로에 착륙합니다.

계약에 의해 읽기 전용. 의도적 인 의미 (양식 양식을 클릭, 국가를 mutate 버튼을 클릭), `/automate`로 및 경로.

모든 페이지 반환은 공격적인 영향력을 입력 (#2441):
...
`/scrape`가 의도한 후 사용자 요청입니다. 그들은 아무도 포함하지 않았다면 한 번 묻습니다.

> "당신은 긁는 것을 원합니까? 한 줄에 설명, 예를 들어 '최고 이야기
> Hacker News 또는 '제품명 + 가격 example.com/products'

프론트에 여러 가지 질문을하지 마십시오. 더 많은 질문은 프로토 타입 경로에 이동합니다.

## Step 2 — mutating intents를 재사용

If the intent implies writes — verbs like *submit*, *post*, *send*, *로그인*, *X를 누릅니다.*, *본문 바로가기*, *delete*, *의 확장*, *order*, *book* — respond:

> "/scrape는 읽기 전용입니다. mutating 흐름을 위해, /automate (browser-skills)를 사용하십시오
> 단계 2 P0 에서 TODOS.md — 아직 발송되지 않음). 그 때까지, 사용 $B 클릭 /
> $B 채우기 / $B 형 직접."

중지. 경기 또는 프로토 타입 경로에 입력하지 마십시오.
...
각 기술에 대해 `$B skill show <name>`는 `triggers:`, `description:`, `host:`를 포함하여 전체 SKILL.md를 노출합니다. 이들을 읽고 사용자의 의도적인 세분화가 그들 중 하나에 일치한다는 것을 판단하십시오.

confident match는 **모든 3**가 true를 의미합니다.

- intent의 도메인은 기술 `host` (또는 호스트 이름 중 하나)에 일치합니다.
- `triggers:` 구문 또는 `description:`는 동일한 자료를 포함합니다
  intent 요청
- intent는 기술이 `args:`에 선언하지 않는다는 주장이 요구되지 않습니다.

일치하면, 의도적으로 `--arg key=value`를 파고 (또는 0arg 기술에 대한 아무도 통과) 및 실행 :

```bash
$B skill run <name> [--arg key=value ...]
```

JSON를 강조하는 스킬 프린트를 stdout로 바꿔 놓습니다. 중지합니다.

일치하면 야심 (두 개의 기술이 엄밀하게 맞을 수 있음), 더 좁은 계층 하나를 선택 (프로젝트 > 글로벌 > 번들 - `$B skill list`는 계층을 보여줍니다). 아직 주변이 많으면, 잘못 추측보다 프로토 타입 경로로 떨어졌습니다.

## 단계 4 — Prototype 단계

일치하지 않습니다. `$B` primitives를 사용하여 페이지를 구동하십시오.

1. `$B goto <url>` - 대상으로 이동. 사용자의 의도는 보통
   호스트 또는 URL 이름을 지정합니다. 직접 사용하십시오.
2. `$B snapshot --text` (또는 `$B text`) - 깨끗한 텍스트보기를 얻을
   selectors를 찾아보기
3. `$B html` - 구조화된 자료를 파는 필요 때 원료 HTML를 당깁니다
   (표, 테이블, 반복 행).
4. `$B links` - 의도가 URL을 수집하는 경우.
5. Iterate: selector를 시도하고, 출력을 검사, 정제.

stdout (하나의 문서, 예쁘지 않은)에 JSON로 결과를 옮깁니다. 안정적인 모양을 사용하세요. 일반적으로 `{ "items": [...], "count": N }` 또는 유사합니다. 따라서 다운스트림 소비자는 데이터로 치료할 수 있습니다.

## 단계 5 — 스킬화 판결

성공적인 프로토 타입 후, 정확히 하나의 라인에 부합:

> "Say /skillify는 이 상설 기술을 만들기 위해 (다음 호출에 200ms)."

즉 전체 판결입니다. 나지 마십시오, 목록 프로가 아니라, 밀어하지 마십시오. Proactive surfacing은 단계 3 손잡이 (`gstack-config browser_skillify_prompts`)이며,이 기술 작업이 아닙니다.

## 시제품이 실패할 때

페이지로드가 아니라 데이터 추출이 감지되지 않는 경우 JSON 모양 3-4 선택 시도 후:

- 당신이 시도한 것을 보고, 무슨 일이, 그리고 무슨 막는 (lazy-loaded,
  JS 렌더링, paywalled, 등).
- NOT 부분 결과를 작성하고 그것을 호출합니다.
- NOT는 부서지는 시제품에 /skillify를 건의합니다.
- 그들이 원하는 사용자에게 (a) 다른 선택자를 시도, (b)
  다른 페이지로 전환, 또는 (c) 정지.

## 이 기술이 NOT는 무엇을 합니다

- 짝지어주는 행동 (사용 /automate 배송시, 또는 $B primitives 직접)
- Auth 흐름 / 쿠키 수입 (사용 /setup-browser-cookies 처음)
- 멀티 페이지 크롤 (이 호출 당 하나 샷입니다)
- 달리는 것은 아니어야 할 일

## 출력 분야

일치 경로는 JSON가 일치한 기술이 방출되는 것을 반환합니다. 시제품 경로는 JSON가 건설하는 것을 반환합니다. 두 경우 모두에서:

- 1개의 JSON 문서, stdout에.
- Stderr (또는 채팅)은 로그 및 스킬화 판결을 위해 사용됩니다.
- 사용자를 제외한 채팅 응답에서 JSON 주위에 prose를 삽입하지 마십시오
  설명에 대해 물었다. — 많은 `/scrape` 콜러 파이프 출력 `jq`.
