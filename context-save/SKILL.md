...
캡처 git 상태, 결정 만든, 그리고 나머지 작업 그래서 어떤 미래 세션을 잃지 않고 픽업 할 수 있습니다. "save progress", "save state", "context save", 또는 "save my work"에 물어. /context-restore와 함께 쌍을 다시 시작. 이전 /checkpoint - Claude Code는 현재 환경에 기본 되감기 별으로 /checkpoint를 치료, 이 기술을 그림으로.
...
**숙련된 직원 엔지니어**입니다. 작업은 전체 작업 컨텍스트를 캡처하는 것입니다. 즉, 결정이 이루어졌는지, 어떤 미래 세션이 (다른 지점이나 직장에서)이 `/context-restore`를 통해 비트를 잃지 않고 다시 시작할 수 있도록합니다.

**HARD GATE:** NOT 코드 변경을 구현합니다. 이 기술은 국가를 캡처합니다.

---

## 명령어를 감지

사용자의 입력을 파로 모드를 결정합니다.

- `/context-save` 또는 `/context-save <title>` → **Save**
- `/context-save list` → **List**

user가 명령(예: `/context-save auth refactor`) 후 제목을 제공하면, 제목으로 사용한다. 그렇지 않으면 현재 작업에서 제목을 입력한다.

`/context-save resume` 또는 `/context-save restore`를 사용한다면, "`/context-restore`를 사용해서 저장하고 복원하는 것은 이제 분리된 기술입니다."
...
수집된 상태와 대화의 역사를 사용하여 요약 덮음을 생성합니다.

1. **무엇을 일하고 있습니까?** - 고급 목표 또는 특징
2. **관련 상품** - 건축 선택, 무역 떨어져, 선택된 접근 및 왜
3. **Remaining 일** - 콘크리트 다음 단계, 우선순위 순서
4. **참고** — 미래의 세션은 알아야 할 사항 (gotchas, blocked items,
   질문, 시도하고 작동하지 않은 것들

사용자가 제목을 제공하면, 그것을 사용합니다. 그렇지 않으면, 작업에서 concise title (3-6 단어)를 입력합니다.

### Step 3: 컴퓨터 세션 기간

이 세션이 얼마나 오래 지속되는지 결정하기 위해 시도하십시오:
...
기간이 결정될 수 없다면, 저장된 파일에서 `session_duration_s` 필드를 굴러 주세요.

### 단계 4: 저장된 텍스트 파일을 쓰기

bash (NOT 의 경로가 LLM 프롬프트)를 컴파일하여 사용자 공급 제목은 쉘 메타 차트를 나중에 명령으로 주사할 수 없습니다. 이 소독제는 허용 목록입니다. `a-z 0-9 - .` 은 생존합니다.
...
on-disk 디렉토리 이름은 `checkpoints/` (`contexts/`)입니다. 이것은 기존의 저장된 파일이 로드할 수 있도록 유지되는 레거시 경로입니다. 사용자는 그것을 볼 수 없습니다.

위의 `$FILE` 경로에 파일을 작성하십시오 (절단 문자열을 사용 - LLM 레이어에서 재구성하지 마십시오).
...
`files_modified` 목록은 `git status --short` (단계로 변경된 파일)에서 옵니다. repo 루트에서 상대적인 경로 사용.
...
**기본 동작:** **현재 branch**만에 저장된 컨텍스트를 표시합니다.

`--all` (예: `/context-save list --all`)를 전달하면 **모든 지점**에서 context를 표시합니다.

`status`, `branch`, `timestamp`를 추출하는 각 파일의 frontmatter를 읽으십시오. filename (배터치 후에 부분)에서 제목을 곱합니다.
...
저장된 컨텍스트가 없다면, 사용자를 말합니다. "저장된 컨텍스트가 아직 없습니다. `/context-save`를 실행하면 현재 작업 상태를 저장합니다."

---

## 중요 규칙

- **비밀번호** 이 기술만 국가를 읽고 컨텍스트 파일을 작성합니다.
- **항상 분기 이름을 포함** frontmatter - 크로스 브레이크에 대한 중요한
  `/context-restore`.
- **저장된 파일은 append-only입니다.** 기존 파일을 덮거나 삭제하지 마십시오. 각
  새 파일을 저장합니다.
- **Infer는, interrogate를 방해하지 않습니다.** git state와 대화 컨텍스트를 사용하여 작성
  파일. 제목이 실제로 불이 없을 경우 AskUserQuestion 만 사용.
- **gstack 기술이 아닌 Claude Code 내장.** 사용자 유형이면
  `/context-save`, 기술 도구를 통해이 기술을 호출합니다. Claude Code의 `/rewind` 별명으로 구성된 오래된 `/checkpoint` 이름은 그 이름을 지정합니다.
