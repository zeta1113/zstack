...
SVG/PNG는 청결한 mermaid 작풍을 이용합니다; .excalidraw는 손으로 그리는 미학을 나릅니다. 완전히 오프라인으로. "도움말을 만들 때 사용", "건축을 그리십시오", "이것으로 바꾸십시오", "이것으로 바꾸십시오", 또는 "이 교류를 활성화하십시오".
...
# /diagram - 영어, 편집 가능한 다이어그램

각 실행은 **triplet**, 절대 죽은 픽셀 덤프를 방출:

| 의약 | 무엇을 위해 |
|---|---|
| `<slug>.mmd` | mermaid 소스 — LLM-친화적인 인터체인지 형식 |
| `<slug>.excalidraw` | 편집 가능한 장면 - excalidraw.com에서 열리고, 상자를 이동하고, 일을 계속하십시오 |
| `<slug>.svg` + `<slug>.png` | docs + raster for chat/issues/READMEs에 대한 crisp 벡터 |

렌더링은 검색 daemon (`lib/diagram-render/dist/diagram-render.html`)의 다이어그램 렌더링 번들을 통해 완전히 오프라인입니다. No CDN, no 네트워크.

## 단계 1 — 저자 다이어그램

사용자의 요청에 대한 mermaid를 작성합니다. 규칙 :

- **유량 (`graph LR`/`graph TD`)**는 단 반점입니다: 그들은 변환합니다
  완전히 편집 가능한 excalidraw 장면. 파이프라인 /flows, `graph TD`를 위한 Prefer `graph LR`.
- 속보, 상태, gantt 및 기타 mermaid 유형은 SVG/PNG 으로 렌더링되지만,
  공식 변환기는 Flowcharts만 지원합니다. `.excalidraw` artifact는 Skipped이고 MUST는 사용자를 말합니다. "sequence diagrams 렌더링 하지만 excalidraw-editable 아직 (upstream converter limit — flowcharts are)"는 없습니다.
- 노드 라벨을 짧은 유지; 가장자리 라벨에 세부 사항을 넣어. 5-15 노드는
  readable 범위. 사용자가 더 많은 것을 요구하면, 여러 다이어그램으로 나누고 왜 말하십시오.

출력 디렉토리를 결정하십시오: `./diagrams/` cwd가 git repo (사용자가 commit), 다른 `/tmp/gstack-diagrams/`일 수 있는 경우에. 다이어그램의 주제 (kebab-case, ≤40 chars)에서 파생한 `<slug>`.

## Step 2 — 렌더링 번들을 단계 ( 세션당 온스)

단계 사본은 내용 취급됩니다 (make-pdf의 전 통행으로 동일 규칙), 그래서 동시 회의 및 혼합 gstack 버전은 각 다른 사람 결코 복제하지 않습니다:
...
`$TAB` - **모든 제품** `$B js` / `$B wait` / `$B closetab` 아래 MUST 패스 `--tab-id $TAB`. 이 없이, 어떤 탭이 활성화되어 있는 경우, /qa 또는 /scrape 세션 공유를 공유할 수 있습니다.

`BUNDLE_MISSING`: 중지 및 사용자 빌드 명령을 표시. CDN fallback을 개선하지 마십시오. 오프라인은 계약입니다.

## 단계 3 — Render 삼중

mermaid 소스를 `<outdir>/<slug>.mmd` 첫 번째 (라이트 도구)로 작성하십시오. 페이지를 읽을 수 없습니다. 그래서 **기초64**를 통해 소스를 발송하지 마십시오. JS 템플릿 리터럴 (백틱스, `${`, 그리고 소스에서 백슬러시가 해석되고 손상 될 것입니다) :
...
참고: `atob()`는 라틴-1을 수확합니다; 비ASCII 상표를 가진 근원을 위해 `decodeURIComponent(escape(atob('…')))`를 정확하게 회복하기 위하여 사용하십시오.

mermaid가 오류를 반환하면 사용자가 파스 오류를 표시하고, mermaid를 수정하고, 재발 - 사용자가 부서진 소스 파일을 수동하지 않습니다. `__mermaidToExcalidraw`가 비-flowchart 유형에 실패하면 `.excalidraw` artifact를 건너 단계 1에서 제한주의로 나머지를 전달합니다.

## Step 4 - 쇼와 전달

1. Read tool을 사용하여 PNG를 읽어보시고, 사용자는 다이어그램 인라인을 볼 수 있습니다.
2. 트리플 경로를 나열합니다.
3. 일렬로 세울 수 있는 편집성 참고: "`.excalidraw` 파일은 excalidraw.com에서 열립니다.
   (File → Open) - 편집하고 편집 장면에서 렌더링 할 수 있습니다. "
4. 사용자가 변경을 원하면 `.mmd` 소스 및 재 실행 단계 3 -
   소스는 진실의 단일 소스입니다.

EDITED `.excalidraw` (사용자 왕복)를 재출구하십시오: mermaid를 접촉하지 않고 장면 파일과 수출을 다시, 장면 JSON가 따옴표와 backslashes의 가득 차있는이기 때문에, base64 수송을 다시 적재하십시오:
...
- **렌더링하지 않고 삼중을 결코 배워하지 마십시오.** A `.mmd` 파일은 혼자 있지 않습니다
  그림. 렌더링이 불가능하다면 (불이 누락된, 찾아보기), 이렇게 말하고 중지합니다.
- **청소:** 대화의 다이어그램 작업이 있을 때 렌더링 탭을 닫습니다.
  행 (`$B closetab $TAB`), 도표 사이 아닙니다.
- PDF에 대해 정의된 다이어그램을 위해 `make-pdf`가 렌더링한 사용자를 상기시킵니다.
  ` ```mermaid ` fences natively — embedding the `.mmd`는 PNG를 삽입하는 것보다 더 낫습니다.

## 완료 상태

- DONE — 트리플 (또는 SVG/PNG 쌍 + 제한 참고) 배달 및 표시.
- BLOCKED - 번들 또는 사용 불가피; build/setup 명령을 표면으로 처리.
