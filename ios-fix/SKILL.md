...
/ios-qa에 의해 발견된 버그를 가져 와서 소스를 읽고, 수정, 재건, redeploys를 작성하고 실제 장치에 대한 수정을 검증합니다. 루프를 닫습니다. 버그 → 수정 버그 → 수정 수정을 확인 - 0 인간 개입. 회귀 테스트 정착물으로 사전 bug state 스냅 샷 캡처, 그래서 버그가 조용히 재발 할 수 없습니다. 사용 /ios-qa 버그를보고 자동으로 고정 할 때, 또는이 "autofix"로 요청할 때,이 아이폰 OS "autofix"를 "autofix"로 요청할 때.

음성 트리거 (speech-to-text aliases) : "iOS 버그를 설정", "아이폰 앱을 실행", "자동 설정 iOS 문제".
...
**NO FIX WITHOUT REPRODUCING SNAPSHOT.** Swift 소스를 편집하기 전에 에이전트 MUST는 버그를 재현하는 `GET /state/snapshot`를 캡처합니다. 스냅 샷은 회귀 테스트 정착물 (`test/fixtures/ios-fix/`)이됩니다. 스냅 샷을 재생하지 않고 토지가 3 개월 만에 다시 배치 될 수 있도록 수정합니다.

## 단계 1: 버그를 수정

1. `/ios-qa` 을 읽으십시오 (버그 설명, 스크린 샷, 의심
   accessibility-tree 노드).
2. `POST /tap`, `/swipe`, `/type`를 통해 버그 상태로 장치를 가져오십시오,
   또는 `POST /state/<key>` (snapshot-eligible 필드에만 해당).
3. 캡처 `GET /state/snapshot` → 쓰기
   `test/fixtures/ios-fix/<bug-slug>-pre.json`.
4. 캡처 `GET /screenshot` → 쓰기
   `test/fixtures/ios-fix/<bug-slug>-pre.png`.
5. 잘못된 것 + 예상 행동의 한 줄 설명 주장.

## Phase 2: 루트 원인을 찾습니다

`/investigate`의 철 법: 뿌리 원인 없이 아무 수정도. 에이전트은 전망 모형, 자료 교류 및 국가 쾌락에 버그 스크린에서 추적을 읽습니다. 행동을 고치는 가장 작은 변화를 식별합니다.

AskUserQuestion를 사용하여 여러 개의 플루시블 루트가 발생하면 사용자가 수정할 것을 선택합니다.

## Phase 3: 수정을 적용

1. Swift 소스를 편집합니다. diff를 최소한으로 유지하십시오.
2. 재건 : `xcodebuild -scheme <SchemeName>
   -destination 'platform=iOS,id=<UDID>' 빌드 설치`.
3. Daemon은 StateServer 터널을 재건하고 재연결합니다.
4. Re-deploy. 동일한 부팅 - 투켄 회전 흐름 실행.

## 4 단계 : 검증

1. `POST /state/restore` pre-bug snapshot →를 사용하여 상태를 재현합니다.
2. 신선한 스크린 샷을 가져 가라. 비교
   `test/fixtures/ios-fix/<bug-slug>-pre.png`.
3. 버그가 보이지 않는 persists 경우, 수정은 작동하지 않았다 — 역동적이고 다시 시도
   (사용자에 확장하기 전에 최대 3 반복).
4. 버그가 사라지면, 회귀 테스트의 `<bug-slug>-post.png`를 캡처합니다.

## 단계 5: 회귀 시험 추가

`test/fixtures/ios-fix/<bug-slug>.test.ts`에서 테스트를 작성합니다.

1. 사전 bug 스냅샷을 로드합니다.
2. `POST /state/restore`를 통해 복원합니다.
3. 실제 기기의 포스트 수정 동작을 증명합니다 (gated)
   `GSTACK_HAS_IOS_DEVICE=1`, 주기적인 층).

snapshot fixture + 테스트 파일을 수정과 함께 수정합니다.

## 실패 모드

| 의약 | Action |
|---|---|
| 3 침략, 버그는 여전히 존재 | STOP, 현재 제일 hypothesis를 가진 사용자에 보고 |
| `409 schema_mismatch` 에 /state/restore 재건축 후 | Re-codegen accessors (`swift run gen-accessors`), 재 냅킨 |
| 장치가 중간 고정 | daemon 자동 연결; 단계 4에서 이력서 |
| 실패를 | Revert Swift 편집; 재 승인 수정 전에 컴파일 오류를 조사 |
