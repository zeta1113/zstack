...
쿠키 도메인을 가져 오는 것을 선택하면 대화 형 피커 UI를 엽니다. QA 테스트 인증 페이지의 사용. "import Cookies", "login to site", 또는 "authenticate the browser".
...
실제 Chromium 브라우저에서 headless 검색 세션으로 로그인 세션을 가져옵니다.

## CDP 모드 체크

먼저, 검색이 이미 사용자의 실제 브라우저에 연결되는 경우 확인:
```bash
$B status 2>/dev/null | grep -q "Mode: cdp" && echo "CDP_MODE=true" || echo "CDP_MODE=false"
```
`CDP_MODE=true`: "필요하지 않는 경우, CDP를 통해 실제 브라우저에 연결됩니다. 쿠키 및 세션은 이미 사용할 수 있습니다." 그리고 중지. 쿠키 가져 오기가 필요하지 않습니다.

## 어떻게 작동합니까?

1. 검색하기
2. `cookie-import-browser`를 실행하여 설치한 브라우저를 감지하고, 피커 UI를 엽니다.
3. 사용자들은 쿠키 도메인을 선택하여 브라우저에 가져갈 수 있습니다.
4. 쿠키는 Playwright 세션으로 해독되고 로드됩니다.
...
이 자동 감지는 Chromium 브라우저를 설치하고 기본 브라우저에서 대화식 테이너 UI를 엽니다.
- 설치된 브라우저의 전환
- 검색 영역
- 도메인의 쿠키를 가져오기 위해 "+"를 클릭하십시오
- 수입된 쿠키를 제거하려면 trash를 클릭하십시오

사용자를 말하십시오: **"쿠키 피커가 열렸다. 브라우저에서 가져와야 할 도메인을 선택하고, 당신이 행해질 때 말해줘. "**

##3. 직접 수입 (대략)

사용자가 직접 도메인을 지정하는 경우 (예: `/setup-browser-cookies github.com`), UI를 건너뛰기:

```bash
$B cookie-import-browser comet --domain github.com
```

지정된 경우 `comet`를 적절한 브라우저로 바꾸십시오.
...
- macOS에서 브라우저 당 첫 번째 가져 오기가 키 체인 대화 상자를 트리거 할 수 있습니다. "Allow"/ "Always Allow"를 클릭하십시오.
- Linux에서 `v11` 쿠키는 `secret-tool`/libsecret 접근을 요구할 수 있습니다. `v10` 쿠키 사용 Chromium의 표준 fallback 키
- 쿠키 선택기는 검색 서버 (추가 프로세스 없음)와 같은 포트에 제공됩니다.
- 도메인 이름과 쿠키 수만 UI에 표시되어 있습니다. 쿠키 값은 노출되지 않습니다.
- 검색 세션은 명령 사이 쿠키를 주장하므로 쿠키가 즉시 작동하도록
