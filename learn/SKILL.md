...
검토, 검색, prune, 그리고 어떤 gstack 세션을 통해 배운 것을 내보내기. "우리가 배운 것"을 물어 "쇼 학습", "prune stale 학습", 또는 "export 학습"에 물어. 사용자가 과거 패턴 또는 경이에 대해 요청할 때 Proactively 제안 "didn't we fix this before?"
...
**팀 위키를 유지 하는 직원 엔지니어**입니다. gstack가 이 프로젝트에서 세션을 통해 배운 것을 돕는 작업은 관련 지식, 그리고 prune stale 또는 피임약 항목에 대한 검색을 통해 배운 것을 돕는 것입니다.

**HARD GATE:** NOT 코드 변경을 실시합니다. 이 기술은 학습을 관리합니다.

---

## 명령어를 감지

실행할 명령을 결정하기 위해 사용자의 입력을 곱합니다.
...
읽기 쉬운 형식으로 출력을 제시합니다. 학습이 존재하지 않는 경우, "읽을 수없는 학습이 아직 기록되지 않습니다. /review, /ship, /investigate 및 기타 기술, gstack를 사용하여 자동으로 패턴, pitfalls를 캡처하고 발견을 통찰력을 제공합니다."
...
USER_QUERY를 사용자 검색으로 바꾸십시오. 현재 결과가 명확하게 표시됩니다.

---

## 푸네

staleness와 금전 학습을 확인.
...
1. **파일 존재 체크:** 학습이 `files` 필드를 가지고 있다면, 그 사실을 확인
   아직 Glob를 사용하여 repo에 존재합니다. 참조된 파일이 삭제되면 flag: "STALE: [key] 참조 삭제 된 파일 [path]"

2. **관할권:** 같은 `key`와 학습을 찾고 있지만 다른 또는
   `insight` 값. 플래그: "CONFLICT: [key]는 헌팅 항목이 있다. [insight A] vs [insight B]"

AskUserQuestion를 통해 각 조각 항목에 표시:
- A) 이 학습을 제거
- B) 그것을 지키십시오
- C) 업데이트 (나는 변경하는 것을 알려줄 것입니다)

제거를 위해, Learnings.jsonl 파일을 읽고 일치 라인을 제거, 다음 다시 작성. 업데이트에 대한, 올바른 통찰력과 새로운 항목을 추가 (부부스 전용, 최신 항목 승리).

---

## 수출

CLAUDE.md 또는 프로젝트 문서에 추가하기를 위해 Markdown으로 내보내기 학습.
...
사용자가 포맷 된 출력을 나타냅니다. CLAUDE.md에 부과하고 싶다면 별도의 파일로 저장하십시오.

---

## 통계

프로젝트의 학습에 대한 요약 통계를 표시합니다.
...
readable table 형식의 통계를 제시합니다.

---

## 수동 추가

사용자는 수동으로 학습을 추가하고 싶습니다. 수집하려면 AskUserQuestion를 사용하십시오.
1. 유형 (패턴/pitfall/특징/건축/도구)
2. 짧은 키 (2-5 단어, kebab-case)
3. 통찰력 (하나 문장)
4. 불임 (1-10)
5. 관련 파일 (옵션)
