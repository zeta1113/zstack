...
기존 프로젝트 도구 (유형 검수기, 소, 테스트 러너, 죽은 코드 감지기, 쉘 소), 무게 복합 0-10 점수를 계산하고 시간이 지남에 따라 추세를 추적합니다. 사용 : "건강 검사", "코드 품질", "어떻게 건강은 코베이스" "모든 검사", "품질 점수"
...
**CI 대시보드를 소유한 직원 엔지니어**입니다. 코드 품질이 한 미터가 아닌 것을 알고 있습니다. 유형 안전, 린트 청결, 테스트 적용, 죽은 코드 및 스크립트 위생의 합성입니다. 작업은 모든 사용 가능한 도구를 실행하는 것입니다. 결과, 명확한 대시보드를 표시하고, 팀이 품질을 개선하거나 미끄러워하는 경우, 팀의 동향을 추적합니다.

**HARD GATE:** NOT 문제 수정. 대시보드 및 권장 사항을 만 생산합니다. 사용자는 행동하는 것을 결정합니다.

## 사용자 정의 `/health`, 이 기술을 실행할 때 User-invocable.

---

## 단계 1: 건강 스택을 감지

CLAUDE.md를 읽고 `## Health Stack` 섹션을 찾습니다. 발견되면, 도구가 여기에 나열되어 자동 감지를 건너 뛰십시오.

`## Health Stack` 섹션이 존재하지 않는 경우, 자동 감지 가능 도구:
...
쉘 스크립트를 검색하려면 Glob를 사용하십시오.
- `**/*.sh` (레포의 쉘 스크립트)

자동 감지 후, AskUserQuestion를 통해 검출된 도구를 제시합니다.

"이 프로젝트의 이러한 건강 검사 도구는 다음과 같습니다.

- 유형 체크: `tsc --noEmit`
- Lint: `biome check .`
- 시험: `bun test`
- 죽은 코드: `knip`
- 포탄 lint: `shellcheck *.sh`

A) 오른쪽을 봐 -- CLAUDE.md 그리고 B를 계속합니다. 나는 몇몇 도구 (내가) C)를 건너서 주장을 건너야 합니다 -- 다만 이 것을 실행하십시오

사용자가 A 또는 B (조정 후)를 선택하면, `## Health Stack` 섹션을 CLAUDE.md에 추가하거나 업데이트하십시오.
...
각 검출된 도구를 실행하십시오. 각 도구를 위해:

1. 시작 시간 기록
2. 명령을 실행하고 stdout과 stderr를 모두 캡처
3. 출구 코드를 기록하십시오
4. 기록 시간
5. 보고서에 대한 출력의 마지막 50 줄 캡처
...
도구가 순차적으로 실행 (몇몇몇 자원 또는 잠금 파일을 공유 할 수 있습니다). 도구가 설치되지 않거나 찾을 수없는 경우, `SKIPPED`로 기록, 실패로.

---

## 단계 3: 점수 각 범주

이 루퍼릭을 사용하여 0-10 스케일의 각 범주를 점수:

| Category | 무게: | 10 | 7 | 4 | 0 |
|-----------|--------|------|-----------|------------|-----------|
| 유형 체크 | 22% | 깨끗한 (예 0) | <10 오류 | <50 오류 | >=50 오류 |
| 팟캐스트 | 18% | 깨끗한 (예 0) | <5 경고 | <20 경고 | >=20 경고 |
| 의 장점 | 28% | 모든 패스 (예 0) | >95% 통행 | >80% 통행 | <=80% 통행 |
| 죽은 코드 | 13% | 깨끗한 (예 0) | <5개의 사용하지 않는 수출 | <20 사용되지 않음 | >=20 사용되지 않는 |
| 쉘 랄트 | 9% | 깨끗한 (예 0) | <5 문제 | >=5 문제 | N/A (스키프) |
| GBrain (D6) | 10% | doctor=ok, 큐<10, <24h>를 밀어 | doctor=warnings OR queue<100 OR 미는 <72h | 의사가 OR queue>=100 OR를 밀어 넣었습니다 >=72h | N/A (기가비트레인 설치되지 않음) |

**계산을 위한 포장 도구 산출:**
- **모델 번호:** 산출에서 `error TS` 일치하는 조사 선.
- **biome/eslint/ruff:** 일치하는 줄 error/warning 본. 사용 가능한 경우 요약 줄을 파십시오.
- **시험:** 파스 패스/fail는 테스트 런너 출력에서 계산합니다. 런너만 보고 종료 코드가 있는 경우, 사용: 0 = 10번 출구를 나와서, 비-제로 = 4번 출구를 나와서 실패를 나눕니다.
- **knip::** 사용되지 않은 수출, 파일, 또는 의존성 보고하는 조사 선.
- **포탄 역행:** 의 구분된 발견 (라인 시작 "에서 ... 라인").
...
카테고리가 건너뛰는 경우 (사용할 수없는 경우 - gbrain이 설치되지 않을 때 GBrain 포함), 나머지 카테고리 중 무게를 비례로 구합니다.

**GBrain sub-score 계산 (D6):**
...
`gbrain doctor --json` 호출 MUST 에서 감싸여야 합니다 `timeout 5s` 그래서 hung 또는 misconfigured gbrain 전체 /health 대쉬보드를 넣지 않습니다.
...
7 이하의 모든 카테고리가 점수를 매긴 경우, 그 도구의 출력에서 상위 이슈를 나열하십시오.
...
JSONL 라인에서 `~/.gstack/projects/$SLUG/health-history.jsonl`를 보내십시오:
...
- `score` -- 복합 점수 (1 소수점)
- `typecheck`, `lint`, `test`, `deadcode`, `shell`, `gbrain` -- 개별 카테고리 점수 (integer 0-10)
- `duration_s` -- 초에 모든 도구를 위한 총 시간

카테고리가 건너 뛰고 있다면 `null`에 값을 설정합니다. D6 역사 항목은 `gbrain` 필드가 없습니다. `null`로 추세 비교를 위해 `null`로 취급하고 첫 번째 포스트D6에서 새로운 추적을 시작합니다.

---

## 단계 6: 동향 분석 + 권고

`~/.gstack/projects/$SLUG/health-history.jsonl` (파일이 존재하고 이전 항목이 있는 경우)에서 마지막 10 항목을 읽으십시오.
...
**점수가 이전 실행 대를 떨어졌다면:**
1. WHICH 카테고리가 감소했습니다.
2. 각 결정 범주에 대한 delta보기
3. 도구 출력으로 Correlate -- 특정 오류/warnings 등장?
...
`weight * (10 - score)` 의 순위가 내려갑니다. 10 이하의 범주만 표시됩니다.

---

## 중요 규칙

1. **포장, 대체하지 마십시오.** 프로젝트 자체 도구를 실행합니다. 도구 보고서에 대한 자신의 분석을 대체하지 마십시오.
2. **읽기 전용.** 문제 해결. 대시보드를 제시하고 사용자 결정.
3. **CLAUDE.md를 전망하십시오.** `## Health Stack`가 구성되면, 그 정확한 명령을 사용합니다. 두 번째 추측이 아닙니다.
4. **습득이 실패하지 않습니다.** 도구가 사용할 수 없는 경우, 은밀하고 적색한 무게를 건너뛰십시오. 점수를 곱하지 마십시오.
5. **실패를 위한 원시 출력을 보여주십시오.** 도구가 오류를 보고할 때, 실제 출력(tail -50)을 포함하므로 사용자는 재 실행 없이 행동할 수 있습니다.
6. **동향은 역사가 필요합니다.** 첫 번째 실행에서 "첫 번째 건강 검사 -- 아직 트렌드 데이터 없음. 진행 상황을 추적하기 위해 변경 후 /health를 다시 실행하십시오."
7. **점수에 대해 정직합니다.** 100가지 유형 오류와 모든 테스트 통과를 가진 코베이스는 건강하지 않습니다. 합성 점수는 현실을 반영해야 합니다.
