...
Claude, GPT (Codex CLI), 그리고 Gemini 측에 의하여 동일한 프롬프트를 달립니다 — LLM 판단을 통해 대기권, 토큰, 비용 및 선택적으로 질을 비교합니다. vibes 대신 자료로 "모델이 실제로 가장 우수하다"를 대답하십시오. /benchmark에서 분리되는, 웹 페이지 성과를 측정하는. 사용: "benchmark 모형", " 비교 모형", ", 모형"는 X-model를 위한 가장 좋은 비교 모형"입니다.

음성 트리거 (speech-to-text aliases) : "모델 비교", "모델 촬영", "모델은 최고입니다".
...
# /benchmark-models - 크로스 모델 스킬 벤치 마크

`/benchmark-models` 워크플로우를 실행하고 있습니다. `gstack-model-benchmark` 이진을 `gstack-model-benchmark`로 감싸고, 공급자를 확인하고, auth를 미리 확인하고 벤치 마크를 실행합니다.

`/benchmark`와 다른 기술 측정 웹 페이지 성능 (Core Web Vitals, 로드 타임). 이 기술 측정 AI 모델 성능 gstack 기술 또는 임의 프롬프트.
...
찾을 수 없다면, 중지하고 사용자가 gstack을 다시 설치합니다.

---

## Step 1: 프롬프트를 선택하십시오.

preamble 형식과 AskUserQuestion를 사용하십시오:
- **재 배경:** 현재 프로젝트 + branch.
- **의욕:** "크로스 모델 벤치 마크는 2-3 AI 모델을 통해 동일한 프롬프트를 실행하고 속도, 비용 및 출력 품질에 비교하는 방법을 보여줍니다. 우리가 사용하는 것은 무엇입니까?
- **RECOMMENDATION:** 실제 기술에 대한 벤치마킹 때문에 도구 사용 차이를 노출, 그냥 원시 생성.
- **옵션:**
  - A) 벤치 마크 내 gstack 기술 중 하나 (우리는 다음 기술을 선택할 것입니다). 완료 : 10 / 10.
  - B) 인라인 프롬프트를 사용 - 다음 차례에 입력. 완료 : 8/10.
  - C) 디스크에 신속한 파일에 포인트 - 다음 차례의 경로 지정. 완료: 8/10.

A: SKILL.md 파일이 있는 gstack 기술 목록이면, 사용자가 두번째 AskUserQuestion을 통해 하나를 선택하도록 요청합니다. SKILL.md 경로가 프롬프트 파일로 선택된 SKILL.md 경로를 사용하십시오.

B: 인라인 프롬프트에 대한 사용자를 요청하면. `--prompt "<text>"`를 통해 동사태를 사용합니다.

C: 경로에 대한 요청. 존재를 검증. 위치 인자로 사용.
...
건조한 실행 출력을 표시합니다. "Adapter 가용성"섹션은 공급자가 실제로 실행되는 사용자를 말합니다 (OK) vs Skip (NOT READY - 재약힌 포함).

ALL 3개의 쇼 NOT READY: 명확한 메시지로 멈추십시오 — 벤치마크는 적어도 1개의 오거진 공급자 없이 달리는 달릴 수 없습니다. `claude login`, `codex login`, 또는 `gemini login`/`export GOOGLE_API_KEY`를 건의하십시오.

OK: AskUserQuestion:
- **의욕:** "무게 모델은 포함해야 합니까? authed가 보여준 건식. Unauthed ones는 깨끗하게 건너 뛸 것입니다. 그들은 일괄 처리를 구부리지 않습니다."
- **RECOMMENDATION:** A (모든 authed 공급자)는 가능한 한 많은 부유 한 비교를 제공 하기 때문에.
- **옵션:**
  - A) 모든 오제 공급자. 완료: 10/10.
  - B) 단지 클로드. 완료: 6/10 (no 크로스 모델 신호 - 대신 솔로 클로드 벤치 마크에 대한 /ship의 검토를 사용하십시오.
  - C) 다음 차례로 두 가지를 선택. 완료: 8/10.
...
심사위원이 유효하다면 AskUserQuestion:
- **의욕:** "품질 판단은 안토픽의 Claude를 타이 브레이커로 사용하여 0-10 스케일에 각 모델의 출력을 점수합니다. 출력 품질에 대해 걱정할 경우 ~ $0.05/run.을 추가하면 대기 시간 및 비용이 들지 않습니다."
- **RECOMMENDATION:** A — 전체적인 점은 질을, 다만 속도 비교하고 있습니다.
- **옵션:**
  - A) 적법한 판단 (adds ~ $ 0.01). 완료 : 10/10.
  - B) Skip 판단 - speed/cost/tokens만. 완료: 7/10.

심사자가 NOT인 경우, 이 질문을 건너 `--judge` 플래그를 구합니다.

---

## 단계 4: 벤치 마크를 실행

단계 1, 2, 3 결정에서 명령을 지시하십시오.

```bash
"$BIN" <prompt-spec> --models <picked-models> [--judge] --output table
```

`<prompt-spec>`는 `--prompt "<text>"` (Step 1B), 파일 경로 (Step 1A 또는 1C), `<picked-models>`는 단계 2.에서 comma-separated 명부입니다.

출력을 도착으로 스트리밍합니다. 이것은 느리다 — 각 공급자는 완전히 신속한 실행합니다. 선명한 복잡성에 따라서 30s-5min를 예상하고 `--judge`가 켜져 있는지 여부.
...
- **퀵메뉴** - 최저 대기 시간으로 제공.
- **Cheapest** - 최저 비용으로 제공.
- **높은 품질** (`--judge` ran) - 가장 높은 점수를 가진 공급자.
- **의 모든 것** - 판단을 사용합니다. 판단이 ran: 품질 무게를 달면. 그렇지 않으면: 사용자의 요구를 정의하는 거래에 주의하십시오.

어떤 공급자가 오류를 명중하면 (auth/timeout/rate_limit), 구제 경로로 호출합니다.
...
AskUserQuestion:
- **의욕:** "이 벤치 마크를 JSON로 저장하므로 미래의 실행을 비교할 수 있습니까?"
- **RECOMMENDATION:** A - 공급자로 기술 성과 편류는 그들의 모형을 새롭게 합니다; 저장된 기본은 질 회귀를 붙잡습니다.
- **옵션:**
  - A) `~/.gstack/benchmarks/<date>-<skill-or-prompt-slug>.json`에 저장하십시오. 완료: 10/10.
  - B) 다만 인쇄, 저장하지 마십시오. 완료: 5/10 (향상 자료를 잃으십시오).

A: `--output json`와 티를 가진 재 실행되는 경우에 날짜 파일에. 이렇게 경로를 인쇄하십시오 diff 미래는 그것에 대하여 실행할 수 있습니다.

---

## 중요 규칙

- **Step 2의 Dry-run이 없는 실제 벤치 마크를 결코 실행하지 마십시오.** 사용자는 API 호출을 보내는 전에 auth 상태를 볼 필요가 있습니다.
- **절대 하드 코드 모델 이름.** 항상 사용자의 단계 2 선택에서 공급자를 전달합니다. - 바이너리는 나머지를 처리합니다.
- **자동 포함하지 않는 `--judge`.** 실제 비용을 추가합니다. 사용자는 선택해야 합니다.
- **제로 제공자가 authed 경우, STOP.** 벤치 마크를 시도하지 마십시오. - no 유용한 출력을 생성합니다.
- **비용도 볼 수 있습니다.** 각 런은 테이블에 있는 per-provider 비용을 보여줍니다. 사용자는 다음 실행의 앞에 그것을 볼 수 있어야 합니다.
