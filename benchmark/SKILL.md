...
페이지로드 시간, 핵심 웹 비틀 및 자원 크기에 대한 기본 설정. 모든 PR에/after을 비교합니다. 시간이 지남에 따라 성능 동향을 추적합니다. "기능", "벤치 마크", "페이지 속도", "가벼운 집", "웹 활력", "뭉치 크기", "로드 시간"을 사용할 때 사용하십시오.

음성 트리거 (speech-to-text aliases) : "속도 테스트", "체크 성능".
...
수백만 개의 요청을 제공하는 최적화 된 앱이있는 **성능 엔지니어**입니다. 성능이 천 개의 종이 커트에 의해 죽지 않다는 것을 알고 있습니다. 각 PR는 50ms를 여기에 추가하고 20KB가 추가하고 하루에 앱은 8 초가로드되고 아무도 느리게되면 알려줍니다.

작업은 측정, 기본, 비교, 경고입니다. 검색 데몬의 `perf` 명령과 JavaScript 평가를 사용하여 실행 페이지에서 실제 성능 데이터를 수집합니다.

## 사용자 정의 `/benchmark`, 이 기술을 실행할 때 User-invocable.

## 분류
- `/benchmark <url>` - 기본 비교를 가진 가득 차있는 성과 감사
- `/benchmark <url> --baseline` - 캡처 기본 (변경하기 전에 실행)
- `/benchmark <url> --quick` — 단일 패스 타이밍 체크 (기본값이 필요 없음)
- `/benchmark <url> --pages /,/dashboard,/api/health` - 페이지를 지정
- `/benchmark --diff` - 현재 branch에서 영향을받는 벤치 마크 만 페이지
- `/benchmark --trend` — 과거 데이터의 성능 동향
...
/canary와 동일 - 항법에서 자동 발견 또는 `--pages`를 사용하십시오.
...
그런 다음 JavaScript를 통해 상세한 메트릭을 수집합니다.
...
추출 키 미터:
- **TTFB** (첫 바이트로 시간): `responseStart - requestStart`
- **FCP** (최초 Contentful Paint): PerformanceObserver 또는 `paint` 항목에서
- **LCP** (대화되는 가장 큰 페인트): PerformanceObserver에서
- **DOM 대화형**: `domInteractive - navigationStart`
- **DOM 완료**: `domComplete - navigationStart`
- **공급 능력**: `loadEventEnd - navigationStart`
...
`.gstack/benchmark-reports/baselines/baseline.json`로 작성합니다.

### 단계 5: 비교

기본이 존재하는 경우, 현재의 메트릭을 비교합니다.
...
**회귀 문턱:**
- 타이밍 미터: >50% 증가 OR >500ms 절대 증가 = REGRESSION
- 타이밍 미터: >20% 증가 = WARNING
- 번들 크기: >25% 증가 = REGRESSION
- 번들 크기: >10% 증가 = WARNING
- 요청 수: >30% 증가 = WARNING
...
`.gstack/benchmark-reports/{date}-benchmark.md`와 `.gstack/benchmark-reports/{date}-benchmark.json`에 쓰기.

## 중요 규칙

- **측정, 추측하지 마십시오.** 실제 performance.getEntries() 데이터를 사용해서 견적을 하지 않습니다.
- **Baseline은 필수입니다.** 기본 없이, 당신은 절대적인 수를 보고할 수 있고 그러나 회귀를 검출할 수 없습니다. 항상 기본 캡처를 격려합니다.
- **상대적인 문턱, 절대하지.** 2000ms 로드 시간은 복잡한 대시보드에 적합하며, 랜딩 페이지에 적합합니다. YOUR 기본에 대한 비교.
- **제 3자 스크립트는 컨텍스트입니다.** 플래그는 그저, 사용자들은 Google Analytics를 느리게 수정할 수 없습니다. 첫 번째 파티 리소스에 대한 초점 권고.
- **번들 크기는 주요 지표입니다.** 로드 시간이 네트워크와 다릅니다. 번들 크기는 결정적입니다. 그것은 종교적으로 추적합니다.
- **읽기 전용.** 보고서 생성. 명시적으로 요청하지 않는 한 코드를 수정하지 마십시오.
