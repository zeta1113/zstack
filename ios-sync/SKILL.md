...
업데이트 StateServer.swift, DebugOverlay.swift, Package.swift, 그리고 @Observable 상태 접근자 입력. 당신은 gstack를 업그레이드한 후에 사용하거나 액세스 허용이 필요한 새로운 ViewModels/properties를 추가하십시오. "iOS 디버그 브리지 동기화"를 요청할 때, "iOS accessors를 재생하거나, "gstack iOS 계측을 업데이트하십시오.

음성 트리거 (speech-to-text aliases) : "iOS 디버그 브리지 동기화" "iOS 액세스", "update the gstack iOS 계측".
...
`/ios-qa`가 앱에 설치되면, 사용자는 다음을 할 수 있습니다.

1. 새로운 `@Observable` 클래스 또는 속성을 추가하여 액세스 권한을 필요로 합니다.
2. gstack를 더 열심히 하는 수정을 가진 새로운 버전으로 업그레이드하십시오.
3. `// @Snapshotable` 발전기 마커를 다른 필드에 붙여넣으십시오.

이 기술은 관련 기사를 게재합니다.

**템플릿은 업스트림 gstack에서 살고 있습니다.** 설치 `gstack-ios-qa-regen` 발사자는 `ios-qa/templates/`에서 지원되는 교량 파일만 그것의 자신의 gstack 뿌리를 해결하고 사본을. 포크의 HTTP-fetch와 wildcard 복사 본은 사라집니다.

## Phase 1: 설치된 버전을 검출하십시오

1. `<app>/DebugBridgeGenerated/.gstack-version` (/ios-qa에 의해 쓰기
   설치 중). 누락된 경우, "unknown old version"로 설치를 치료합니다.
2. `$GSTACK_ROOT/VERSION`에서 업스트림 버전을 읽으십시오.
3. AND 새 `@Observable` 클래스가 추가된 경우, 종료
   "일시까지"로 일찍.

## Phase 2: 재생산 코겐 산출

한 번 세례적인 regenerator를 실행하십시오. `--app-source`는 accessor 스캐너가 검사되어야 합니다. `--bridge-dir`는 Debug 구조에 있는 앱 링크가 있는 로컬 Swift 패키지입니다:
...
명령은 현재 접속자를 방출하기 전에 전 편평한 `DebugBridgeGenerated/` 배치에서 알려진 사용되지 않은 생성된 파일만 제거합니다. 생성은 파일경보 관측 가능한 종류 및 JSON-native 사기그, 배열, 문자열 키가 있는 사전 및 선택적인 분야 유형을 받아들입니다. 그것은 주문 유형, implicitly unwrapped 선택적인, 배열된 관찰 가능한 종류 및 완료 감적을 쓰기 전에 중복 스냅샷 열쇠를 거절합니다.

복합 해시 캐시 키는 실제로 재생해야하는지 여부를 처리합니다. Swift 버전, 발전기 git rev, lockfile, 소스 콘텐츠 및 플랫폼이 캐시와 일치하면 ~50ms no-op입니다.

## Phase 3: 생성 된 디프를 검토

1. `<app>/DebugBridge/` 및 `<app>/DebugBridge/`의 변경 사항 및
   `<app>/DebugBridgeGenerated/StateAccessor.swift`.
2. 명령을 확인하면 app의 Handwritten Swift 파일을 수정하지 않았습니다.
3. 앱 대상에 앱 별 배선을 유지; canonical bridge package files
   업스트림에서 재생되며, 손 편집되지 않아야합니다.

## 4 단계 : 검증

1. `swift build` 앱 패키지에 대해 성공합니다.
2. `xcodebuild -scheme <SchemeName>` 성공.
3. 장치에서 앱을 다시 시작; daemon는 + 토큰을 회전합니다.
4. `GET /state/snapshot` 새 액세스자 schema 해시를 반환합니다.

## 실패 모드

| 의약 | Action |
|---|---|
| Swift 컴파일은 regen 후 실패 | `git restore` + AskUserQuestion를 통해 재회: 컴파일 오류를 표면 |
| Codegen은 잘못된 표시된 선언을 보고합니다. | JSON-native type, internal/public setter, 그리고 모형의 열쇠 고유를 가진 파일경 관측 가능한 종류 및 writable 인스턴스 `var`를 사용하십시오; 그렇지 않으면 `// @Snapshotable` 감적을 제거합니다. |
| Schema hash는 새로운 @Observable 추가한 후에 변하지 않았습니다 | 필드는 독립 `// @Snapshotable` 마커 코멘트가 없습니다. - 코겐은 제대로 말하지 않는 상태를 제외합니다. 스냅 샷해야하는 각 필드의 즉시 의견을 추가하십시오. |
| 스캐너는 생성 된 교량 소스를 참조하십시오. | 좁은 앱 소스 디렉토리를 전달합니다. regenerator는 `DebugBridgeGenerated` 및 `StateAccessor.swift`를 자동으로 제외합니다. |
