...
배포 플랫폼 (Fly.io, Render, Vercel, Netlify, Heroku, GitHub Actions, custom), 생산 URL, 건강 체크 엔드포인트 및 배포 상태 명령을 감지합니다. CLAUDE.md에 구성을 작성하면 모든 미래 배포가 자동됩니다. "setup deploy", "configure deploy", "set up land-and-deploy", "gstack"로 배포하는 방법을 "configure deploy"를 사용하십시오.
...
# /setup-deploy - gstack를 위한 배포 구성

`/land-and-deploy` 은 자동으로 배포를 구성하는 데 도움이 됩니다. 작업은 배포 플랫폼, 생산 URL, 건강 검사 및 배포 상태 명령을 감지하는 것입니다. - 다음 CLAUDE.md 에 모든 것을 주장합니다.

`/land-and-deploy`가 CLAUDE.md를 읽고, 전적으로 탐지를 건너 뛰기 후에.

## 사용자 정의 `/setup-deploy`, 이 기술을 실행할 때 User-invocable.
...
구성이 이미 존재하면, 그것을 표시하고 물어라.

- **구성 :** 이미 CLAUDE.md에 존재합니다.
- **RECOMMENDATION:** 설정이 변경된 경우 업데이트하기 위해 A를 선택합니다.
- A) 스크래치에서 재구축 (여기)
- B) 특정 필드 편집 (show current config, 나를 변경 한 가지)
- C) Done - 구성은 올바른
...
어떤 검출이 되었는지, 플랫폼별 구성을 통해 사용자를 안내합니다.
...
2. `fly` CLI가 설치된 경우, `which fly 2>/dev/null`
3. 설치시, 확인시: `fly status --app {app} 2>/dev/null`
4. 인퍼 URL: `https://{app}.fly.dev`
5. 배포 상태 명령 설정: `fly status --app {app}`
6. 건강 검사를 놓으십시오: `https://{app}.fly.dev` (또는 `/health`는 app가 1인 경우에)

URL를 확인하려면 사용자를 요청하십시오. 일부 Fly 앱은 사용자 정의 도메인을 사용합니다.

#### 렌더링

`render.yaml`가 검출된 경우:

1. render.yaml에서 서비스 이름과 유형 추출
2. API 키 체크: `echo $RENDER_API_KEY | head -c 4` (전체 키를 노출하지 마십시오)
3. 인퍼 URL: `https://{service-name}.onrender.com`
4. Render는 연결된 지점으로 자동 배포합니다. - 작업 흐름이 필요하지 않습니다.
5. 건강 검사를 놓으십시오: inferred URL

사용자를 확인하도록 요청하십시오. 렌더링은 연결 된 git 분기에서 자동 배포를 사용하여 메인으로 병합 한 후 자동으로 선택합니다. /land-and-deploy의 "deploy wait"은 새 버전과 반응 할 때까지 URL 렌더링을 poll해야합니다.

#### 버셀

vercel.json 또는 .vercel가 검출된 경우:

1. `vercel` CLI: `which vercel 2>/dev/null`를 위해 검사하십시오
2. 설치시: `vercel ls --prod 2>/dev/null | head -3`
3. Vercel은 푸시에 자동으로 배치합니다. PR, 주요 합병에 대한 생산
4. 건강 검사 설정: 생산 URL vercel 프로젝트 설정에서

#### Netlify에 대한 의견

netlify.toml가 검출된 경우:

1. netlify.toml에서 사이트 정보 추출
2. Netlify는 푸시에 자동으로 배치합니다.
3. 건강 검사 설정: 생산 URL

### GitHub 동작

워크플로우를 감지하지만 플랫폼 구성이 없습니다.

1. 작업 흐름 파일을 읽어서 어떻게 해야 하는지 이해하기
2. 배포 대상을 추출 (추가되는 경우)
3. URL 생산에 대한 사용자를 요청하십시오.

#### 사용자 정의 / 매뉴얼

어떤 검출하지 않는 경우:

AskUserQuestion를 사용하여 정보를 수집합니다.

1. **트리거 된 방법은 무엇입니까?**
   - A) 자동적으로 주요 (Fly, Render, Vercel, Netlify, 등)에 밀어
   - B) GitHub 작업 흐름을 통해
   - C) 배포 스크립트 또는 CLI 명령을 통해 (이제)
   - D) 수동으로 (SSH, 대쉬보드, 등)
   - E) 이 프로젝트는 배포되지 않습니다 (리브러리, CLI, 도구)

2. **URL는 무엇입니까?** (무료 텍스트 — URL 앱이 실행되는 곳)

3. **gstack가 어떻게 성공했는지 확인할 수 있습니까?**
   - A) HTTP 특정 URL (예: /health, /api/status)의 건강 검사
   - B) CLI 명령 (예: `fly status`, `kubectl rollout status`)
   - C) GitHub 작업 흐름 상태를 확인
   - D) 자동화된 방법이 없습니다 — 다만 URL 짐

4. **어떤 전 merge 또는 포스트 merge 걸이?**
   - merging 전에 실행하는 명령 (예 : `bun run build`)
   - 병합 후 실행 명령을 실행하지만, 배포 검증 전에

### Step 4: 구성을 쓰기

CLAUDE.md (또는 그것을 창조하십시오)를 읽으십시오. 그것이 존재하는 경우에 `## Deploy Configuration` 단면도를 찾아서 대체하고, 또는 끝에 그것을 부를 수 있습니다.
...
1. 건강 검사 URL가 구성되면, 시도하십시오:
```bash
curl -sf "{health-check-url}" -o /dev/null -w "%{http_code}" 2>/dev/null || echo "UNREACHABLE"
```

2. 배포 상태 명령이 구성된 경우, 시도해 주세요:
```bash
{deploy-status-command} 2>/dev/null | head -5 || echo "COMMAND_FAILED"
```

결과 보고. 실패한 경우, 메모를 하지 말고, 차단하지 마십시오 — config는 여전히 건강 검사가 일시적으로 제한되지 않는 경우에도 유용합니다.
...
- **비밀을 노출하지 마십시오.** 전체 API 키, 토큰 또는 암호를 인쇄하지 마십시오.
- **사용자 확인.** 항상 검출된 구성을 보여주고 쓰기 전에 확인을 요청합니다.
- **CLAUDE.md는 진실의 근원입니다.** 모든 구성은 별도의 구성 파일에 없습니다.
- **의원** /setup-deploy 여러 번 실행하여 이전 구성을 정리합니다.
- **Platform CLIs는 선택 사항입니다.** `fly` 또는 `vercel` CLI가 설치되지 않은 경우 URL 기반 건강 검사로 다시 떨어질 수 있습니다.
